...
class TestMovementIntercept(unittest.TestCase):
    ...
    def test_GIVEN_movement_and_beam_at_the_same_angle_within_tolerance_WHEN_get_intercept_THEN_raises_calc_error(
        self,
    ):
        angle = 12.3
    ...
    def test_GIVEN_movement_perpendicular_to_z_at_beam_angle_10_WHEN_get_intercept_THEN_position_is_z_as_initial_y_as_right_angle_triangle(
        self,
    ):
        y = 0
        z = 10
    ...
    def test_GIVEN_beam_perpendicular_to_z_at_movement_angle_10_WHEN_get_intercept_THEN_position_is_z_as_initial_y_as_right_angle_triangle(
        self,
    ):
        y = 0
        z = 10
...
class TestMovementRelativeToBeam(unittest.TestCase):
    def test_GIVEN_movement_along_y_WHEN_set_position_relative_to_beam_to_0_THEN_position_is_at_intercept(
        self,
    ):
    ...
    def test_GIVEN_movement_along_y_WHEN_set_position_relative_to_beam_to_10_THEN_position_is_at_10_above_intercept(
        self,
    ):
        movement = LinearMovement(0, 10, 90)
        beam_intercept = Position(0, 10)
    ...
    def test_GIVEN_movement_at_30_to_z_WHEN_set_position_relative_to_beam_to_10_THEN_position_is_at_10_along_intercept(
        self,
    ):
        ...
        assert_that(
            ...
            is_(
                position(
                    Position(beam_intercept.y + dist / 2.0, beam_intercept.z + dist * sqrt(3) / 2.0)
                )
            ),
        )
