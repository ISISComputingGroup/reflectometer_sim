...
from src.ioc_driver import HeightDriver, TiltingJawsDriver
...
class TestHeightAndAngleDriver(unittest.TestCase):
    def setUp(self):
        ...
        start_position_angle = 90.0
        max_velocity_angle = 10.0
        self.angle_axis = MagicMock()
        self.angle_axis.name = "JAWS:ANGLE"
        self.angle_axis.value = start_position_angle
        self.angle_axis.max_velocity = max_velocity_angle

        self.tilting_jaws = TiltingJaws("component", movement_strategy=LinearMovement(0.0, 10.0, 90.0))

        self.jaws_driver = TiltingJawsDriver(self.tilting_jaws, self.height_axis, self.angle_axis)

    def test_GIVEN_multiple_axes_need_to_move_WHEN_computing_move_duration_THEN_maximum_duration_is_returned(self):
        beam_angle = 45.0
        expected = beam_angle / self.angle_axis.max_velocity
    ...
    def test_GIVEN_move_duration_and_target_position_set_WHEN_moving_multiple_axes_THEN_computed_axis_velocity_is_correct_and_setpoint_set_for_all_axes(self):
        ...
        expected_velocity_angle = 4.5
        target_position_angle = 135.0
        self.tilting_jaws.set_incoming_beam(beam)
        self.tilting_jaws.set_position_relative_to_beam(0.0)  # move component into beam

        self.jaws_driver.perform_move(target_duration)

        assert_that(self.height_axis.velocity, is_(expected_velocity_height))  # TODO why fail?
        assert_that(self.height_axis.value, is_(target_position_height))
        assert_that(self.angle_axis.velocity, is_(expected_velocity_angle))
        assert_that(self.angle_axis.value, is_(target_position_angle))
