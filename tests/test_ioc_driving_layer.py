...
class BeamlineMoveDurationTest(unittest.TestCase):
    def test_GIVEN_multiple_components_in_beamline_WHEN_triggering_move_THEN_components_move_at_speed_of_slowest_axis(self):
        ...
        slit_2_pos = TrackingPosition("s2_pos", slit_2, sim=True)
        slit_3_pos = TrackingPosition("s3_pos", slit_3, sim=True)
        det_pos = TrackingPosition("det_pos", detector, sim=True)
        mode = BeamlineMode("mode name", [smangle.name, slit_2_pos.name, slit_3_pos.name, det_pos.name])
        beamline = Beamline([supermirror, slit_2, slit_3, detector], [smangle, slit_2_pos, slit_3_pos, det_pos], [supermirror_driver, slit_2_driver, slit_3_driver, detector_driver], [mode])
